...
jaMakeRounded = function (elements, exclude) {
	if (!exclude) exclude = '';
	elements = $$(elements);
	if (!elements || !elements.length) return;
	
	var top = new Element ('div', {'class':'top'});
	new Element('div',{'class':'tl'}).inject (top);
	new Element('div',{'class':'tc'}).inject (top);
	new Element('div',{'class':'tr'}).inject (top);
	var bot = new Element ('div', {'class':'bot'});
	new Element('div',{'class':'bl'}).inject (bot);
	new Element('div',{'class':'bc'}).inject (bot);
	new Element('div',{'class':'br'}).inject (bot);
	var wrap = new Element('div', {'class':'jarounded clearfix'});
	elements.each (function(el) {
		var elw = wrap.clone().injectAfter(el).adopt(el.remove());
		if (!exclude.test ('top')) top.clone().injectTop (elw);
		if (!exclude.test ('bottom')) bot.clone().inject (elw);
	});
}